...
import com.baro.utils.parsing.MenuExtraToMenu.MenuExtraToMenu;
import com.baro.utils.parsing.MenuExtraToMenu.MenuExtraToMenuList;
...
public class MenuExtraExemptToMenuPageController implements Initializable {
    public Button menu_extra_delete_button;
    @FXML
    ...
    private void setCategoryList(String result) {
        ...
        AnchorPane header = new AnchorPane();
        Label categoryNameLabel = new Label("category_name");
            ...
            thisMenu.selectedProperty().addListener(new ChangeListener<Boolean>() {
                ...
                public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
                    if(newValue) {
                        ...
                        String result = ConnectToServer.Get("ExtraByMenuPrint.do?menu_id="+menu.menu_id);
                        if(GetBool.getBool(result)) {
    ...
    private void makeMenuExtraList(String result, int menuId) {
        MenuExtraToMenuList menuExtraToMenuList = new Gson().fromJson(result, MenuExtraToMenuList.class);
        if(!menuExtraToMenuList.result || menuExtraToMenuList.extraByMenus == null) {
        ...
        for (int i = 0; i < menuExtraToMenuList.extraByMenus.size() ; i++) {
            final int index = i;
            MenuExtraToMenu menuExtraToMenu = menuExtraToMenuList.extraByMenus.get(i);
            AnchorPane content = new AnchorPane();
            Text id = new Text(menuExtraToMenu.id+"");
            Text extraPriceText = new Text(menuExtraToMenu.extra_price+"");
            Text extraNameText = new Text(menuExtraToMenu.extra_name);
            Text extraMaxCountText = new Text(menuExtraToMenu.extra_maxcount+"");
            CheckBox thisMenuExtra = new CheckBox();

            content.getChildren().addAll(id, extraPriceText, extraNameText, extraMaxCountText, thisMenuExtra);
            content.getChildren().get(1).setLayoutX(100);
            content.getChildren().get(2).setLayoutX(200);
            content.getChildren().get(3).setLayoutX(300);
            content.getChildren().get(4).setLayoutX(400);

            thisMenuExtra.selectedProperty().addListener(new ChangeListener<Boolean>() {
                @Override
                public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
                    if(newValue) {
                        menu_extra_delete_button.setOnAction(new EventHandler<ActionEvent>() {
                            @Override
                            public void handle(ActionEvent event) {
                                if(GetBool.getBool(ConnectToServer.Get("DeleteExtraByMenu.do?id="+menuExtraToMenu.id))) {
                                    menuExtraToMenuList.extraByMenus.remove(index);
                                    menu_extra_list.getItems().remove(index + 1);
                                    System.out.println("삭제성공");
                                    if(menuExtraToMenuList.extraByMenus.size() == 0) {
                                        menu_extra_list_vbox.setVisible(false);
                                    }
                                }
                                else {
